...
macro get_c_fun (dlopen_lib, to_name, from_fun)
    ...
    arguments = map(function (arg) #Whine about stuff done wrong. Convert arguments if needed.
    ...
    body = quote 
             $(Expr(:ccall, cat(1, {:(dlsym($dlopen_lib, #Really needs to be a quote.
             $(Expr(:quote,{c_name},Any)))), 
             return_type, Expr(:tuple,input_types,Any)},
             argument_names), Any))
           end
    ret  = Expr(:function,
             {Expr(:call, cat(1,{to_name},argument_names),Any), body}, Any)
           return esc(ret)
end
